...
const CHU_DE_MAP: Record<string, string> = { VOCABULARY: 'Từ vựng', GRAMMAR: 'Ngữ pháp', LISTENING: 'Nghe', READING: 'Đọc' }

function IconEdit() {
  return <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
}
function IconTrash() {
...
function IconSearch() {
  return <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
}
function IconChevron() {
  return <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
}

function CustomDropdown({ value, onChange, options, placeholder }: { value: string; onChange: (v: string) => void; options: string[]; placeholder: string }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button type="button" onClick={() => setOpen(v => !v)} className="flex items-center gap-3 rounded-[20px] border border-gray-200 bg-white px-4 h-10 text-sm font-medium text-gray-800 hover:border-gray-300 transition-colors min-w-[120px] justify-between shadow-sm">
        <span>{value || placeholder}</span><IconChevron />
      </button>
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1 min-w-[130px] overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
          <button type="button" onClick={() => { onChange(''); setOpen(false) }} className="block w-full px-4 py-2.5 text-left text-sm text-gray-600 hover:bg-gray-50">{placeholder}</button>
          {options.map(opt => (
            <button key={opt} type="button" onClick={() => { onChange(opt); setOpen(false) }} className="block w-full px-4 py-2.5 text-left text-sm hover:bg-gray-50" style={value === opt ? { backgroundColor: '#E9D5FF', color: '#7C3AED', fontWeight: 600 } : undefined}>{opt}</button>
          ))}
        </div>
      )}
    </div>
...
export default function FlashcardTable({ decks, total, page, pageSize, levels }: Props) {
  ...
  const hasPublishedSelected = Array.from(selected).some(id => decks.find(x => x.id === id)?.isPublish)
  ...
  async function handleBulkPublish() {
    setLoading(true)
    try {
      await Promise.all(Array.from(selected).map(id => fetch(`/api/flashcard/bo-the/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isPublish: true }) })))
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-4 items-center mb-6">
        <div className="relative w-[320px]">
          ...
          <input type="text" value={searchInput} onChange={e => setSearchInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder="Tìm kiếm flashcard" className="w-full pl-11 pr-4 h-10 text-sm font-medium border border-gray-200 rounded-[20px] focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary shadow-sm" />
        </div>
        <CustomDropdown value={levelFilter} onChange={setLevelFilter} options={levels} placeholder="Cấp độ" />
        <CustomDropdown value={''} onChange={() => { }} options={['Từ vựng', 'Ngữ pháp']} placeholder="Chủ đề" />
      ...
      <div className="overflow-hidden rounded-[20px] border border-gray-200 bg-white shadow-sm">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-[#F3F4F6] text-left">
              <th className="px-4 py-4 w-10 border-b border-gray-200">
                <input type="checkbox" checked={allSelected} onChange={e => setSelected(e.target.checked ? new Set(decks.map(d => d.id)) : new Set())} className="w-5 h-5 rounded-[4px] border-gray-300 cursor-pointer transition-colors" style={{ accentColor: PRIMARY }} />
              </th>
              <th className="px-4 py-4 font-bold text-gray-600 border-b border-gray-200">Tên deck</th>
              ...
              <th className="px-4 py-4 font-bold text-gray-600 border-b border-gray-200">Số thẻ</th>
          ...
          <tbody>
            {decks.map(deck => {
              ...
              return (
                <tr key={deck.id} className="border-b border-gray-100 last:border-0 transition-colors hover:bg-gray-50" style={isChecked ? { backgroundColor: '#FDF0FF' } : undefined}>
                  <td className="px-4 py-4">
                    <input type="checkbox" checked={isChecked} onChange={e => { const s = new Set(selected); e.target.checked ? s.add(deck.id) : s.delete(deck.id); setSelected(s) }} className="w-5 h-5 rounded-[4px] border-gray-300 cursor-pointer transition-colors" style={{ accentColor: PRIMARY }} />
                  </td>
                  <td className="px-4 py-4 font-medium text-gray-900">{deck.tieuDe}</td>
                  <td className="px-4 py-4 font-medium text-gray-900">{deck.level ?? '—'}</td>
                  <td className="px-4 py-4 font-medium text-gray-900">Từ vựng</td>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleDeleteSingle(deck.id)} title="Xóa" disabled={loading} className="flex items-center justify-center w-8 h-8 rounded-md hover:bg-red-50 transition-colors text-gray-600 hover:text-red-500 disabled:opacity-50"><IconTrash /></button>
                    </div>
                  </td>
                </tr>
              )
            })}
            {decks.length === 0 && <tr><td colSpan={8} className="px-4 py-12 text-center text-sm text-gray-400">Chưa có bộ thẻ nào</td></tr>}
          </tbody>
        </table>
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-white border-t border-gray-100">
          <div className="flex items-center gap-4 min-h-[36px]">
            {selected.size > 0 ? (
              <>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-bold text-gray-900">Đã chọn {selected.size} deck</span>
                </div>
            ...
            ) : <span className="text-sm text-gray-400 font-medium">{total} bộ thẻ</span>}
          </div>
          {totalPages > 1 && (
            <div className="flex items-center gap-1.5">
              <button onClick={() => router.push(buildUrl({ page: page - 1 }))} disabled={page <= 1} className={`flex h-7 w-7 items-center justify-center rounded-[6px] transition-all ${
              ...
              <div className="flex items-center gap-1">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                  <button key={p} onClick={() => router.push(buildUrl({ page: p }))} className={`flex h-7 min-w-[28px] items-center justify-center rounded-md text-xs transition-all ${
              ...
              <button onClick={() => router.push(buildUrl({ page: page + 1 }))} disabled={page >= totalPages} className={`flex h-7 w-7 items-center justify-center rounded-[6px] transition-all ${
